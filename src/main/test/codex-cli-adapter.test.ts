...
import { extractJsonEventSummary, translateRunnerEventToChunk } from '../adapters/codex-cli.adapter'
...
describe('translateRunnerEventToChunk', () => {
  ...
  describe('json-event translation', () => {
    ...
    it('translates function_call_output', () => {
      const result = translateRunnerEventToChunk(jsonEvent({ type: 'function_call_output' }), now)
    ...
    it('routes object error field to stderr with red [error] prefix', () => {
      const result = translateRunnerEventToChunk(jsonEvent({ error: { message: 'timeout' } }), now)
      expect(result?.type).toBe('stderr')
