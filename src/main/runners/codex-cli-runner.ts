...
export async function buildCodexExecArgs(
  ...
  for (const [key, value] of [...configEntries.entries()].sort(([a], [b]) => a.localeCompare(b))) {
...
function mapCodexExitError(
  ...
  if (
    /(codex login|not authenticated|authentication|required login|please log in)/i.test(
      combinedOutput
    )
  ) {
  ...
  public async *run(ctx: RunnerContext): AsyncGenerator<RunnerEvent, RunnerResult, void> {
    ...
    const handleJsonLine = (line: string, forceTrailingNewline: boolean): void => {
      ...
      try {
        ...
        queue.push(
          createRunnerEvent('stdout', forceTrailingNewline ? `${normalizedLine}\n` : normalizedLine)
        )
      }
    }
