...
function detectReadmeTargetUsers(readme: string | null): string | undefined {
  ...
  const startIndex = lines.findIndex((line) => {
    ...
    return (
      normalized.includes('target users') ||
...
export const nodeDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      ...
      ['package.json', 'src', 'app', 'pages', 'packages', 'apps'].filter(
    ...
    addUnique(
      ...
      [
        ...
        'app/page.tsx',
        'pages/index.tsx'
      ].filter((item) => snapshot.hasFile(item))
    )
    if (hasAnyDirectory(snapshot, ['src/main', 'src/preload', 'src/renderer'])) {
      ...
      addUnique(
        result.importantPaths,
        ['src/main', 'src/preload', 'src/renderer'].filter((item) => snapshot.hasDirectory(item))
      )
      addUnique(result.architectureParts, [
        'Electron app split across main, preload, and renderer surfaces.'
      ])
    ...
    if (result.verificationCommands.length === 0) {
      addUnique(result.riskFlags, [
        'No package verification scripts were detected in package.json.'
      ])
    }
    ...
    addEvidence(
      ...
      'package.json',
      'high',
      'Derived from Node dependencies and scripts.',
      [...packages]
    )
    if (result.verificationCommands.length > 0) {
      addEvidence(
        ...
        'package.json',
        'high',
        'Derived from available package scripts.',
        Object.keys(scripts)
      )
    }
    return result
...
export const javaDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    const gradleFiles = snapshot.files.filter((file) =>
      ['build.gradle', 'build.gradle.kts', 'settings.gradle', 'settings.gradle.kts'].includes(
        file.name
      )
    )
    ...
    addUnique(
      result.importantPaths,
      ['src/main/java', 'src/main/resources', 'src/test/java'].filter((item) =>
        snapshot.hasDirectory(item)
      )
    )
    ...
    if (hasMaven) {
      ...
      addUnique(result.verificationCommands, [
        snapshot.hasFile('mvnw.cmd') ? '.\\mvnw.cmd test' : 'mvn test'
      ])
      ...
      addEvidence(
        ...
        'pom.xml',
        'high',
        'Derived from Maven dependencies.',
        signals
      )
    }
    ...
    if (gradleFiles.length > 0) {
      const buildFile =
        gradleFiles.find((file) => file.name.startsWith('build.gradle')) ?? gradleFiles[0]
      ...
      addUnique(
        result.importantPaths,
        gradleFiles.map((file) => file.relativePath)
      )
      addUnique(result.verificationCommands, [
        snapshot.hasFile('gradlew.bat') ? '.\\gradlew.bat test' : 'gradle test'
      ])
      ...
      addEvidence(
        result,
        'primaryStack',
        buildFile?.relativePath ?? 'build.gradle',
        'high',
        'Derived from Gradle build files.'
      )
    }
    ...
    if (snapshot.hasDirectory('src/main/java')) {
      const boundaries = [
        ...
        snapshot.directories.some(
          (directory) =>
            directory.relativePath.endsWith(`/src/main/java/${name}`) ||
            directory.relativePath.endsWith(`/src/main/java/com/${name}`) ||
            directory.name === name
        )
      )
      if (boundaries.length > 0) {
        addUnique(
          result.moduleBoundaries,
          boundaries.map((name) => `Java ${name} layer`)
        )
        addUnique(result.architectureParts, [
          'Layered Java backend with application, domain, service, persistence, and configuration code.'
        ])
      } else {
    ...
    addUnique(
      ...
      snapshot.files
        .filter(
          (file) =>
            file.relativePath.startsWith('src/main/java/') && file.name.endsWith('Application.java')
        )
        .map((file) => file.relativePath)
    )
    if (result.entrypoints.length > 1) {
      addUnique(result.riskFlags, [
        'Multiple Java application entrypoints were detected; bootstrap package may need review.'
      ])
      addUnique(result.recommendedFirstActions, [
        'Review duplicate Java application entrypoints before feature work.'
      ])
    }
    addEvidence(
      result,
      'verificationCommands',
      hasMaven ? 'pom.xml' : (gradleFiles[0]?.relativePath ?? 'build.gradle'),
      'high',
      'Derived from Java build wrapper and build tool.'
    )
    return result
  }
}
...
export const pythonDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      result.importantPaths,
      ['pyproject.toml', 'requirements.txt', 'Pipfile', 'manage.py', 'src', 'tests'].filter(
        (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
      )
    )

    if (snapshot.hasFile('poetry.lock') || snapshot.hasFile('pyproject.toml')) {
      addUnique(result.packageManagers, [
        snapshot.hasFile('poetry.lock') ? 'Poetry' : 'Python packaging'
      ])
    ...
    if (combinedSignals.includes('django') || snapshot.hasFile('manage.py')) {
      ...
      addUnique(
        result.entrypoints,
        ['manage.py'].filter((item) => snapshot.hasFile(item))
      )
    }
    ...
    if (combinedSignals.includes('pytest') || snapshot.hasDirectory('tests')) {
      ...
      addUnique(result.verificationCommands, [
        snapshot.hasFile('poetry.lock') ? 'poetry run pytest' : 'python -m pytest'
      ])
    } else {
      addUnique(result.verificationCommands, ['python -m compileall .'])
    }
    addUnique(result.architectureParts, [
      snapshot.hasDirectory('src')
        ? 'Python project with source code under src/.'
        : 'Python project detected from package and source files.'
    ])
    addEvidence(
      result,
      'primaryStack',
      snapshot.hasFile('pyproject.toml') ? 'pyproject.toml' : 'requirements.txt',
      'medium',
      'Derived from Python manifests and source files.'
    )
    return result
  }
}
...
export const goDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      result.importantPaths,
      ['go.mod', 'cmd', 'internal', 'pkg'].filter(
        (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
      )
    )
    addUnique(
      result.entrypoints,
      snapshot.files.filter((file) => file.name === 'main.go').map((file) => file.relativePath)
    )
    addUnique(
      result.moduleBoundaries,
      ['cmd entrypoints', 'internal packages', 'pkg reusable packages'].filter((label) => {
    ...
    addUnique(result.architectureParts, [
      'Go module with package boundaries inferred from cmd/internal/pkg directories.'
    ])
...
export const rustDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      result.importantPaths,
      ['Cargo.toml', 'src', 'crates'].filter(
        (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
      )
    )
    addUnique(
      result.entrypoints,
      ['src/main.rs', 'src/lib.rs'].filter((item) => snapshot.hasFile(item))
    )
...
export const dotnetDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      result.importantPaths,
      [...solutionFiles, ...projectFiles].map((file) => file.relativePath)
    )
    ...
    addUnique(result.architectureParts, [
      '.NET solution or project detected from solution/project files.'
    ])
    addEvidence(
      result,
      'primaryStack',
      solutionFiles[0]?.relativePath ?? projectFiles[0]?.relativePath ?? '*.csproj',
      'high',
      'Derived from .NET solution/project files.'
    )
    return result
  }
}
...
export const phpDetector: ProjectDetector = {
  id: 'php',
  async detect(snapshot) {
    const composer = parseJsonObject<{
      name?: string
      description?: string
    ...
    addUnique(
      result.importantPaths,
      ['composer.json', 'app', 'src', 'tests'].filter(
        (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
      )
    )
    if (packages.has('laravel/framework')) {
      ...
      addUnique(
        result.entrypoints,
        ['artisan'].filter((item) => snapshot.hasFile(item))
      )
    }
    ...
    addUnique(result.verificationCommands, [
      composer.scripts?.test ? 'composer test' : 'vendor\\bin\\phpunit'
    ])
    ...
    addEvidence(
      ...
      'composer.json',
      'high',
      'Derived from Composer dependencies.',
      [...packages]
    )
    return result
  }
...
export const rubyDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    addUnique(
      result.importantPaths,
      ['Gemfile', 'app', 'config', 'spec', 'test'].filter(
        (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
      )
    )
    if (gemfile.includes('rails')) {
      ...
      addUnique(
        result.entrypoints,
        ['config.ru'].filter((item) => snapshot.hasFile(item))
      )
    }
    ...
    addUnique(result.architectureParts, [
      'Ruby project detected from Gemfile and conventional app/config directories.'
    ])
...
export const mobileDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    const hasAndroid =
      snapshot.hasFile('settings.gradle') ||
      snapshot.hasFile('settings.gradle.kts') ||
      snapshot.hasDirectory('android')
    const hasIos =
      snapshot.files.some(
        (file) => file.extension === '.xcodeproj' || file.extension === '.xcworkspace'
      ) || snapshot.hasDirectory('ios')
    const packageJson = parseJsonObject<PackageJsonShape>(await snapshot.readText('package.json'))
    const packages = packageJson ? packageNames(packageJson) : new Set<string>()
    const hasReactNative =
      packages.has('react-native') ||
      (snapshot.hasDirectory('android') && snapshot.hasDirectory('ios') && Boolean(packageJson))
    ...
    if (hasReactNative) {
      ...
      addUnique(
        result.importantPaths,
        ['android', 'ios', 'src'].filter((item) => snapshot.hasDirectory(item))
      )
    }
    if (hasAndroid) {
      ...
      addUnique(
        result.importantPaths,
        ['android', 'build.gradle', 'settings.gradle'].filter(
          (item) => snapshot.hasFile(item) || snapshot.hasDirectory(item)
        )
      )
    }
    ...
    addUnique(result.architectureParts, [
      'Mobile project signals detected from Flutter, Android, iOS, or React Native files.'
    ])
    addEvidence(
      result,
      'primaryStack',
      hasFlutter ? 'pubspec.yaml' : packageJson ? 'package.json' : 'workspace',
      'medium',
      'Derived from mobile framework files.'
    )
    return result
  }
}
...
export const infraDetector: ProjectDetector = {
  id: 'infra',
  async detect(snapshot) {
    const dockerFiles = snapshot.files.filter(
      (file) =>
        fileNameEquals(file, 'Dockerfile') || file.name.toLowerCase().startsWith('docker-compose')
    )
    ...
    const k8sFiles = snapshot.files
      .filter((file) => file.extension === '.yaml' || file.extension === '.yml')
      .filter(
        (file) =>
          file.relativePath.toLowerCase().includes('k8s') ||
          file.relativePath.toLowerCase().includes('kubernetes')
      )
    ...
    if (dockerFiles.length > 0) {
      ...
      addUnique(
        result.importantPaths,
        dockerFiles.map((file) => file.relativePath)
      )
    ...
    if (terraformFiles.length > 0) {
      ...
      addUnique(
        result.importantPaths,
        terraformFiles.slice(0, 5).map((file) => file.relativePath)
      )
    ...
    if (helmFiles.length > 0) {
      ...
      addUnique(
        result.importantPaths,
        helmFiles.map((file) => file.relativePath)
      )
    }
    if (k8sFiles.length > 0) {
      ...
      addUnique(
        result.importantPaths,
        k8sFiles.slice(0, 5).map((file) => file.relativePath)
      )
    }
    ...
    addEvidence(
      result,
      'primaryStack',
      result.importantPaths[0] ?? 'workspace',
      'medium',
      'Derived from infrastructure manifests.'
    )
    return result
  }
}
...
export const monorepoDetector: ProjectDetector = {
  ...
  async detect(snapshot) {
    ...
    const hasCargoWorkspace =
      (await snapshot.readText('Cargo.toml'))?.includes('[workspace]') ?? false
    const hasMavenModules = (await snapshot.readText('pom.xml'))?.includes('<modules>') ?? false
    const hasGradleSettings =
      (await snapshot.readText('settings.gradle'))?.includes('include') ||
      (await snapshot.readText('settings.gradle.kts'))?.includes('include')
    ...
    addUnique(
      ...
      [
        ...
        'settings.gradle',
        'settings.gradle.kts'
      ].filter((item) => snapshot.hasFile(item) || snapshot.hasDirectory(item))
    )
    addUnique(result.moduleBoundaries, ['Workspace apps/packages or multi-module projects'])
    addUnique(result.architectureParts, [
      'Workspace contains monorepo or multi-module project signals.'
    ])
    addUnique(result.riskFlags, [
      'Multiple project roots may exist; choose the active app/package before implementation work.'
    ])
    addUnique(result.recommendedFirstActions, [
      'Identify the primary app/package for the next workflow run.'
    ])
    addEvidence(
      result,
      'moduleBoundaries',
      result.importantPaths[0] ?? 'workspace',
      'medium',
      'Derived from workspace and multi-module files.'
    )
    return result
  }
}
  ...
  async detect(snapshot) {
    ...
    if (result.targetUsers) {
      addEvidence(
        ...
        'README.md',
        'medium',
        'Derived from README audience section.'
      )
    }
    return result
  }
